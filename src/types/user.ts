...
export class User {
	...
	constructor(obj: IUser) {
		...
		if (obj.profile_picture) this.profile_picture = obj.profile_picture;
		else this.profile_picture = 'default/profile-pictures/default.png';
	...
	static toPatch(
		id: string,
		obj: { name?: string; profile_picture?: string },
	): string {
		let update_text = [];

		if (!obj.name && !obj.profile_picture) return '';

		if (obj.name) update_text.push(`name = '${obj.name}'`);
