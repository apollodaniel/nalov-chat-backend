...
export async function message_put_middleware(
	...
	try {
		...
		EVENT_EMITTER.emit(
			`update-${get_users_chat_id(message.receiver_id, user_id.user_id)}`,
		);
...
export async function message_delete_middleware(
	...
	const message = await get_single_message(auth.user_id, message_id);

	if (message.sender_id !== auth.user_id) return resp.sendStatus(401);

	try {
		await delete_message(auth.user_id, message_id);
		EVENT_EMITTER.emit(
			`update-${get_users_chat_id(message.receiver_id, auth.user_id)}`,
		);
		return resp.sendStatus(200);
	} catch (err: any) {
		console.log(err.message);
