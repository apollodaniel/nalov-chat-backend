...
exports.getChunkHeaderEndIndex = getChunkHeaderEndIndex;
exports.receive_file_middleware = receive_file_middleware;
...
function parseMiddleFileChunk(buffer, attachments) {
    ...
    if (!attachment)
        throw new Error(`no matches for filename '${filename}'`);
    attachment.fileStream.write(fileContent);
    if (content_type !== 'application/octet-stream' &&
        content_type !== 'text/plain') {
        (0, messages_1.update_attachment)(new message_1.Attachment({ ...attachment }).toUpdateMimeType(content_type));
        if (content_type === 'application/pdf')
            attachment.fileStream.on('finish', () => generate_file_preview(attachment, () => {
                console.log(`Generated pdf preview to ${attachment.id}`);
            }));
    }
    return filename;
}
function parseFileChunk(buffer, matched_boundaries, attachments) {
    const headerEndIndex = getChunkHeaderEndIndex(buffer);
    if (headerEndIndex === -1)
        throw new Error('could not get header end index');
    const fileHeader = buffer
    ...
    const content_type = contentTypeMatch[0]
        ...
        .replace('\n', '')
        .trim();
    const fileContent = buffer.slice(headerEndIndex, matched_boundaries.length > 1
...
function getChunkHeaderEndIndex(buffer) {
    const buffer_str = buffer.toString('binary');
    const content_type_match = buffer_str.match(/Content-Type:.+/);
    if (!content_type_match || content_type_match.length === 0)
        return -1;
    return (buffer.indexOf(content_type_match[0]) + content_type_match[0].length + 4); // + 8 bytes from \r\n\r\n
}
function updateMimeTypeAndGeneratePreview(buffer, attachment, message) {
    const buffer_str = buffer.toString('binary');
    const content_type_match = buffer_str.match(/Content-Type:.+/);
    ...
    if (mime_type === 'text/plain' || mime_type === 'application/octet-stream')
        ...
        attachment.fileStream.on('finish', () => generate_file_preview(attachment, () => notify_received_attachment(message, attachment)));
    }
    else {
        // adds notify received attachment as callback to filestream on finish
        attachment.fileStream.on('finish', () => notify_received_attachment(message, attachment));
    }
    // update mime type
    (0, messages_1.update_attachment)(new message_1.Attachment({ ...attachment }).toUpdateMimeType(mime_type));
}
function parseChunk(buffer, headers, attachmentStack, message, boundaryCount) {
    ...
    const file_boundary = headers['content-type']
        ...
        .trim();
    const buffer_str = buffer.toString('binary');
    let matched_boundaries_obj = Array.from(buffer_str.matchAll(new RegExp(`-*${file_boundary}-*`, 'g')));
    const matched_boundaries = matched_boundaries_obj.map((match) => match[0]);
    console.log(matched_boundaries);
    let boundaryOcurrences = matched_boundaries_obj.map((match) => match.index);
    ...
    if (matchingOcurrence && firstBoundaryPrevContent.length === 0) {
        ...
        sortedAttachmentStack = attachmentStack.sort((a, b) => a.id === matchingOcurrence[0] ? -1 : 0);
        ...
        if (matchedFilename.length > 1) {
            ...
            updateMimeTypeAndGeneratePreview(buffer, sortedAttachmentStack[0], message); // updates mime type
            const content = buffer.slice(fieldHeaderEndIndex, boundaryOcurrences[1]);
            sortedAttachmentStack[0].fileStream.write(content);
            return parseChunk(buffer.slice(boundaryOcurrences[1], buffer.byteLength), headers, sortedAttachmentStack.filter((_value, index) => index !== 0), // pass the sortedAttachmentStack without first item
            message, boundaryCount);
        }
            ...
            const fieldHeaderEndIndex = getChunkHeaderEndIndex(buffer);
            updateMimeTypeAndGeneratePreview(buffer, sortedAttachmentStack[0], message); // updates mime type
            const content = buffer.slice(fieldHeaderEndIndex, buffer.byteLength);
            sortedAttachmentStack[0].fileStream.write(content);
        ...
        let postEndContent = buffer
            .slice(boundaryOcurrences[0] +
            Buffer.from(matched_boundaries[0]).byteLength, buffer.byteLength)
            .toString('binary')
            .replace('\r', '')
            .replace('\n', '')
            .trim();
        ...
        if (matched_boundaries.length > 1 || postEndContent.length > 0) {
            ...
            let postContentBuffer = buffer.slice(boundaryOcurrences[0], buffer.byteLength);
            return parseChunk(postContentBuffer, headers, sortedAttachmentStack.filter((_value, index) => index !== 0), // removes first attachment from stack
            message, boundaryCount);
        }
        return sortedAttachmentStack.filter((_value, index) => index !== 0);
    }
}
async function receive_file_middleware(req, resp, next) {
    if (!req.headers['content-type'] ||
        !req.headers['content-type']?.startsWith('multipart/form-data'))
        return resp.sendStatus(400);
    const message_id = req.query.message_id?.toString();
    // check message_id
    if (!message_id)
        return resp.sendStatus(401);
    const user_id = new auth_1.Auth({ token: auth_1.Auth.verify_auth_token(req.auth) })
        .user_id;
    const message = await (0, messages_1.get_single_message)(user_id, message_id);
    let attachments = (await (0, messages_1.get_attachments)(message_id)).map((a) => {
        const stream = fs_1.default.createWriteStream(a.path, {
            encoding: 'binary',
            flags: 'a',
        });
        return {
    ...
    if (!fs_1.default.existsSync(dir_path))
        fs_1.default.mkdirSync(dir_path, { recursive: true });
    // raw file stream
    let rawFileStream = fs_1.default.createWriteStream(`${dir_path}/raw_request.raw`, {
        flags: 'a',
        encoding: 'binary',
    });
    let boundaryCount = 0;
    ...
    req.on('data', (data) => {
        ...
        const attStack = [
            ...parseChunk(buffer, req.headers, attachmentStack, message, boundaryCount),
        ];
...
async function generate_file_preview(attachment, callback) {
    const preview_path = `${attachment.path}.png`;
    try {
        const document = await (0, pdf_to_img_1.pdf)(attachment.path, {});
        let preview_page;
        ...
        await fs_1.default.promises.writeFile(preview_path, preview_page, {
            encoding: 'binary',
        });
        (0, messages_1.update_attachment)(new message_1.Attachment({ ...attachment }).toUpdatePreviewPath(preview_path)).then(() => callback());
    }
    catch (err) {
        console.log(`Could not generate preview for attachment ${attachment.filename}\n${err.message}`);
    }
}
function notify_received_attachment(message, attachment) {
    (0, messages_1.update_attachment)(new message_1.Attachment({ ...attachment }).toUpdateDate(Date.now()));
